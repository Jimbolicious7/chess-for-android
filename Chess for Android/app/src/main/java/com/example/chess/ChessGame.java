...
public class ChessGame{
	...
	private boolean frozenPromotion = false;
        ...
        {      
            ...
            else
            {
            		...
            		( board[move.getTo().getRow()][move.getTo().getColumn()] instanceof Pawn))
        		{
					...
					lastRow = move.getTo().getRow();
					lastCol = move.getTo().getColumn();
	...
	public boolean isFrozenPromotion(){
		return frozenPromotion;
	}

	public void promotePawn(char piece){

		switch(piece) {
			case 'Q':
				board[lastRow][lastCol] = new Queen(turn);
				break;
			case 'B':
				board[lastRow][lastCol] = new Bishop(turn);
				break;
			case 'N':
				board[lastRow][lastCol] = new Knight(turn);
				break;
			case 'R':
				board[lastRow][lastCol] = new Rook(turn);
				break;
		}
	...
	public String AIMove(Color color){
		Random random = new Random();
		MoveResult result = MoveResult.Illegal;
		String pMove = "";
		while(result == MoveResult.Illegal) {
			pMove = (char) ((random.nextInt(8) + (int) 'a')) + "" + (random.nextInt(8) + 1) + " " + (char) ((random.nextInt(8) + (int) 'a')) + "" + (random.nextInt(8) + 1);
